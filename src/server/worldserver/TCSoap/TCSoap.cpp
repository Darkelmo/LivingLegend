...
void TCSoapRunnable::run()
{
    ...
    {
        sLog->outError("Soap: не удалось привязать к %s:%d", m_host.c_str(), m_port);
        exit(-1);
    }
    ...
    while (!World::IsStopped())
    {
        ...
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Soap: принято соединение от IP=%d.%d.%d.%d", (int)(soap.ip>>24)&0xFF, (int)(soap.ip>>16)&0xFF, (int)(soap.ip>>8)&0xFF, (int)soap.ip&0xFF);
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Soap: не предоставлена информация для аутентификации");
        return 401;
    }
    ...
    if (!accountId)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Soap: используется неверное имя аккаунта '%s'", soap->userid);
        return 401;
    }

    if (!AccountMgr::CheckPassword(accountId, soap->passwd))
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Soap: неверный пароль для аккаунта '%s'", soap->userid);
        return 401;
    }

    if (AccountMgr::GetSecurity(accountId) < SEC_ADMINISTRATOR)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Soap: %s's уровень доступа слишком низкий", soap->userid);
        return 403;
    }
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "Soap: получена команда '%s'", command);
